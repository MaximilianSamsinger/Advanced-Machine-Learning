...
\begin{document}
	...
	%However there is still a better way to achieve a more evenly distributed, and less random-prone result
	...
	%In \autoref{plot:full4cropped} this corresponds to the purple curve. Allthough since we use randomized functions the lambda values may change each run as we can see in Figure X. 
	
	%On the contrary the overfitting curve is represented by the $\lambda$ value that has the lowest training error, but a comparatively high test error. 
	
	%In the graph this is depicted by the orange curve. Finally the optimal fitting curve is simply the curve with the lowest combined training error and test error, i.e. the best approximation to our initial function.
	
	%All three curves can be seen in Figure X, together with the actual underlying function, the randomized data points and the initial sine function from which we approximated our results.  All of the described curves and values are depicted in \autoref{plot:full4cropped}. 
